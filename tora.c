...
static void tora_moveresize(uint32_t mask, uint32_t* values) {
 ...
 /*tick += (mask & XCB_CONFIG_WINDOW_X);
 tick += (mask & XCB_CONFIG_WINDOW_Y);*/
 if (tick) xcb_configure_window(c, fwin->p, mask, values);
 if (mask & XCB_CONFIG_WINDOW_WIDTH) {
  uint32_t vw[] = { *(values + tick) - 2 * BORDER };
  tick++;
  xcb_configure_window(c, fwin->c, XCB_CONFIG_WINDOW_WIDTH, vw);
  xcb_configure_window(c, fwin->cover, XCB_CONFIG_WINDOW_WIDTH, vw);
 }
 if (mask & XCB_CONFIG_WINDOW_HEIGHT) {
  uint32_t vh[] = { *(values + tick) - TITLE - BORDER };
  xcb_configure_window(c, fwin->c, XCB_CONFIG_WINDOW_HEIGHT, vh);
  xcb_configure_window(c, fwin->cover, XCB_CONFIG_WINDOW_HEIGHT, vh); 
 }
}
...
static void tora_map_notify(xcb_generic_event_t *ev) {
  ...
  xcb_create_window(c, XCB_COPY_FROM_PARENT, dt->p, s->root, 0, 0, 640, 480, 0, XCB_WINDOW_CLASS_INPUT_OUTPUT, s->root_visual, XCB_CW_BACK_PIXEL | XCB_CW_EVENT_MASK, (uint32_t[]){ s->white_pixel, XCB_EVENT_MASK_BUTTON_PRESS | XCB_EVENT_MASK_BUTTON_RELEASE | XCB_EVENT_MASK_EXPOSURE });
  ...
  xcb_create_window(c, XCB_COPY_FROM_PARENT, dt->cover, s->root, 0, 0, 640 - 2 * BORDER, 480 - TITLE - BORDER, 0, XCB_WINDOW_CLASS_INPUT_ONLY, s->root_visual, XCB_CW_EVENT_MASK, (uint32_t[]){ XCB_EVENT_MASK_BUTTON_PRESS | XCB_EVENT_MASK_BUTTON_RELEASE });
  xcb_reparent_window(c, dt->cover, dt->p, BORDER, TITLE);
  ...
  if (state == MOVE) {
    ...
    if (state & UP) {
      ...
      xcb_configure_window(c,
          ...
          v);*/
     tora_moveresize(XCB_CONFIG_WINDOW_HEIGHT | XCB_CONFIG_WINDOW_Y, v/*(uint32_t[]){ g->y + g->height - p->root_y + y, p->root_y - y }*/);
    } else if (state & DOWN) {
      v[0] = p->root_y - g->y;
      xcb_configure_window(c,
          fwin->p,
